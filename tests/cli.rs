...
fn cli_cleanup_off_matches_v0_1_0_output() {
    // The snapshot was generated by the v0.1.0 pipeline, before the cleanup
    // stage existed, on an input cleanup would otherwise change. `--cleanup
    // off` must reproduce it byte for byte.
    let dir = tempfile::tempdir().expect("tempdir");
